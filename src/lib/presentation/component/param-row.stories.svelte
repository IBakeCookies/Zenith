...
<Story
	...
	play={async ({ args, canvas, userEvent }) => {
		// Hover the label: every parameter carries its hint, because none of the names mean anything on
		// their own. The label names the stepper (screen reader and e2e), the help cursor advertises
		// the hint, and the stepper reports in the parameter's own increments.
...
<Story
	...
	play={async ({ canvas }) => {
		// What the user's own logs fit, beside the stepper it would replace, so `Apply my fits`
		// resolves a disagreement they can see
...
<Story
	...
	play={async ({ canvas }) => {
		// Logged, but nothing in them separates this parameter from the default: a fit that failed is
		// not a fit of zero, so no number is printed to invite applying it
