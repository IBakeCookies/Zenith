...
describe('calculateRemainingDay', () => {
	...
	/* A task moved to tomorrow left this day's plan (`isDeferred`), and the re-plan
	   is that plan continued from the hours worked: it funds the row nothing either,
	   or the row reads "spend 2h more" on work the user sent away and can be named
	   next up. Unworked, it never started, so the day owes no switch for it — the
	   reading is the one the day gives without the row. */
	it('never funds a task moved to tomorrow, and charges no switch for one never started', () => {
		const without = calculateRemainingDay(input([SPEC, EMAIL], [[SPEC.id, 1]]));
		...
		expect(without?.hoursByTask.get(EMAIL.id)).toBeGreaterThan(0);
		expect(remaining?.hoursByTask.has(GYM.id)).toBe(false);
		expect(remaining?.nextTask?.id).not.toBe(GYM.id);
		expect(remaining).toEqual(without);
	});

	/* Worked before it was moved, its hours are still the day's: they came off the
	   budget, drew on the pools and started a task the day pays a switch for — the
	   accounting of a task finished and logged, which cannot take more either. */
	it('keeps the hours a moved task was worked spent, switch included', () => {
		const logged: [number, number][] = [
			[SPEC.id, 1],
			[GYM.id, 2],
		];

		const deferred = calculateRemainingDay(
			input(
				[
					SPEC,
					{
		...
		const spent = calculateRemainingDay(
			input(
				[
					SPEC,
					{
		...
		expect(deferred?.workedHours).toBe(3);
		expect(deferred?.remainingHours).toBe(3);
		expect(deferred?.hoursByTask.has(GYM.id)).toBe(false);
		expect(deferred).toEqual(spent);
	});
