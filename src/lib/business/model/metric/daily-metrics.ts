...
export function calculateDailyMetrics(input: DailyMetricsInput): DailyMetrics {
	...
	const planSwitchHours = calculatePlanSwitchHours(plan, switchCost);
	const cognitiveLoad = calculateCognitiveLoad(plan, availableHours);
	...
	return {
		...
		runOrder: new Map(calculateInterleavedOrder(plan).map((task, i) => [task.id, i + 1])),
		planSlackHours,
		planSwitchHours,
		...
		burnoutRisk: calculateBurnoutRisk(plan, availableHours, switchCost, energyParams),
		cognitiveLoad,
		physicalLoad,
		energyBalance: calculateEnergyBalance(cognitiveLoad, physicalLoad),
		frictionIndex: calculateFrictionIndex(plan),
		dailyQuadrant: calculateDailyQuadrant(plan),
		scheduleIntegrity: calculateScheduleIntegrity(plan, availableHours, switchCost),
		...
		deepWorkRatio: calculateDeepWorkRatio(plan, availableHours),
		quickWins: calculateQuickWins(activeTasks),
		grindDensity: calculateGrindDensity(plan),
		rewardDensity: calculateRewardDensity(plan),
		recoveryRatio: calculateRecoveryRatio(plan),
